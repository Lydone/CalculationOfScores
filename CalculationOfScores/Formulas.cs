...
namespace CalculationOfScores {
	static class Formulas {
		...
		public static double[] CalculateProgramming(double[] iw3, double cw3, double[] iw4, double cw4, double hw, double exam, double test) {
			...
			double acc = Math.Round(Math.Round(marks[3] * 0.8, 4) + Math.Round(marks[1] * 0.2, 4));
			double finExam = Math.Round(Math.Round(Math.Min(test, exam) * 0.8, 4) + Math.Round(Math.Max(test, exam) * 0.2, 4));
			marks[4] = Math.Round(Math.Round(acc * 0.2, 4) + Math.Round(finExam * 0.8, 4));
			return marks;
		}
		...
		public static double[] CalculateDiscrete(double hw3, double hw4, double cw3, double cw4, double test) {
			...
			marks[0] = Math.Round(Math.Round(Math.Max(cw3, hw3) * 0.38, 4) + Math.Round(Math.Min(cw3, hw3) * 0.62, 4));
			marks[1] = Math.Round(Math.Round(Math.Max(hw4, cw4) * 0.38, 4) + Math.Round(Math.Min(hw4, cw4) * 0.62, 4));
			marks[2] = Math.Round(Math.Round(Math.Max(marks[0], marks[1]) * 0.38, 4) + Math.Round(Math.Min(marks[0], marks[1]) * 0.62, 4));
			marks[3] = Math.Round(Math.Round(marks[2] * 0.8, 4) + Math.Round(test * 0.2, 4));
			return marks;
		}

		public static double CalculateEnglish(double acc, double exam) {
			return Math.Round(Math.Round(acc * 0.6, 4) + Math.Round(exam * 0.4, 4));
		}
	}
}
